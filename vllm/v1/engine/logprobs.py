...
class LogprobsProcessor:
    ...
    ############优化后处理，简化logprobs返回值
    simple_logprobs: List[Tuple[List[int], List[float]]] = field(default_factory=list)
    ...
    def _update_sample_logprobs_beam_search(self, logprobs_lists: LogprobsLists) -> None:
        """Update with sample logprobs from EngineCore.
...
#                 convert_ids_list_to_tokens(self.tokenizer, token_ids))
            ...
            # Update with the Logprob dictionary for this pos.
#             self.logprobs.append(
#                 self._make_logprob_dict_beam_search(
#                     logprobs,
#                     token_ids,
#                     None,
#                     rank,
#                     self.num_logprobs,
#                 ))
            self.simple_logprobs.append((token_ids, logprobs))
    ...
    @staticmethod
    def _make_logprob_dict_beam_search(
        logprobs: list[float],
        logprob_token_ids: list[int],
        decoded_tokens: Iterable[Optional[str]],
        rank: int,
        ...
        return {
            ...
            for token_id, logprob, rank in zip(
                logprob_token_ids, logprobs, ranks)
        }
    ...
    def update_from_output_beam_search(self, output: EngineCoreOutput) -> None:
        if output.new_logprobs is not None:
            self._update_sample_logprobs_beam_search(output.new_logprobs)
