...
#         prepare_start_time = time.time()
        ...
        #############################
#         print("++prepare inputs spend time: ", time.time() - prepare_start_time)
        # Run the decoder.
        # Use persistent buffers for CUDA graphs.
        with set_forward_context(attn_metadata, self.vllm_config):
            ##################################
#             model_runner_start = time.time()
            output = self.model(
                input_ids=input_ids,
                positions=positions,
...
#             print("****model runner forward spend time: ", time.time() - model_runner_start)
...
#         sample_start_time = time.time()
        ...
        #####################################    
#         print("++sample spend time: ",time.time() - sample_start_time)
...
#         is_idle = stream.query()  # 返回 True 表示流中没有未完成的任务
#         print("GPU 是否有未完成的任务?", "否" if is_idle else "是")
        
#         logprobs_time = time.time()
        logprobs_tensors = sampler_output.logprobs_tensors
        logprobs_lists = logprobs_tensors.tolists() \
            if logprobs_tensors is not None else None
        #########################
#         if logprobs_lists:
#             print("logprobs_list shape: ", logprobs_tensors.logprob_token_ids.shape)
#         print("++logprobs to list spend time: ", time.time() - logprobs_time)
...
#             print("++sampled token tolist spend time: ", time.time() - tolist_time)
...
#             first_sync_time = time.time()
#             torch.cuda.synchronize()  # 确保GPU操作同步
#             print("First sync time (ms):", (time.time() - first_sync_time) * 1000)
            
#             is_idle = stream.query()  # 返回 True 表示流中没有未完成的任务
#             print("GPU 是否有未完成的任务?", "否" if is_idle else "是")

#             transfer_time = time.time()
#             cpu_tensor = sampled_token_ids.cpu()  # 显式转移
#             torch.cuda.synchronize()
#             print("GPU→CPU transfer time:", time.time() - transfer_time)

#             tolist_time = time.time()
#             valid_sampled_token_ids = cpu_tensor.tolist()
#             print("tolist() time:", time.time() - tolist_time)
...
#         print("))))post processing spend time: ", time.time() - sample_start_time)
